...
#include "CrystalPlasticityFccUpdate.h"
#include "CrystalPlasticityStressUpdateBase.h"
#include "libmesh/int_range.h"
#include <cmath>
#include <iostream>

registerMooseObject("irrplasticityApp", CrystalPlasticityFccUpdate);

InputParameters
CrystalPlasticityFccUpdate::validParams()
{
  ...
  params.addParam<Real>("eta", 100.0, "eta");
  params.addParam<Real>("dl", 2.48e-5, "diameter of damage loop");
  params.addParam<Real>("T", 295.0, "temperature");
  params.addParam<Real>("T_critical", 400.0, "critical temperature");
  params.addParam<Real>("loop_num_shock", "shockley_damage_loop");
  params.addParam<Real>("loop_num_frank", "frank_damage_loop");
...
CrystalPlasticityFccUpdate::CrystalPlasticityFccUpdate(
  ...
  : CrystalPlasticityStressUpdateBase(parameters),
    ...
    _dl(getParam<Real>("dl")),
    _T(getParam<Real>("T")),
    _T_critical(getParam<Real>("T_critical")),
    _loop_num_shock(getParam<Real>("loop_num_shock")),
    _loop_num_frank(getParam<Real>("loop_num_frank")),
    ...
    _H_shock(declareProperty<RankTwoTensor>("H_shock")),
    _H_frank(declareProperty<RankTwoTensor>("H_frank")),
    ...
    _H_shock_old(getMaterialPropertyOld<RankTwoTensor>("H_shock")),
    ...
    _previous_substep_H_shock(RankTwoTensor::Identity()),
    ...
    _H_shock_before_update(RankTwoTensor::Identity()),
...
void CrystalPlasticityFccUpdate::computeQpP()
{
  ...
  if (it == _angle_list.end())
  {
    ...
    updateCry();
    _H_shock[_qp] = initH(_loop_num_shock,_amp,_plane_file_name,_cry[_qp], _dl);
    _H_frank[_qp] = initH(_loop_num_frank,_amp,_plane_file_name,_cry[_qp], _dl);
    _H[_qp] = _H_shock[_qp]+_H_frank[_qp];
    _H_list.push_back(_H[_qp]);
    _H_shock_list.push_back(_H_shock[_qp]);
    std::cout<<_angle_list.size()<<_H_list.size()<<std::endl;
  }
  // std::cout<<_angle_list.size()<<_H_list.size()<<std::endl;
}
void CrystalPlasticityFccUpdate::updateCry()
{
  if (_read_prop_user_object)
  {
    _euler_angle[_qp](0) = _read_prop_user_object->getData(_current_elem, 0);
...
void CrystalPlasticityFccUpdate::locateH()
{
  auto it = std::find(_angle_list.begin(), _angle_list.end(), _euler_angle[_qp]);
  if (it == _angle_list.end())
  {
    ...
    _H[_qp]=_H_list[idx];
    _H_shock[_qp]=_H_shock_list[idx];
  
  }
}
void CrystalPlasticityFccUpdate::getSlipSystems()
{
...
std::vector<RealVectorValue> CrystalPlasticityFccUpdate::calplanenorm(const RankTwoTensor & crysrot)
{
  CrystalPlasticityFccUpdate::getSlipSystems();
...
RankTwoTensor initH(Real _number_of_loop,Real _amp,FileName _plane_file_name,RankTwoTensor crysrot, Real _dl)
{
  RankTwoTensor a;
  ...
  for (const auto i : make_range(_number_of_loop))
  {
    ...
    b[i] = (I-outer_product(n_l,n_l))*3.0*_dl;
    a += _amp*b[i];
  }
...
void
CrystalPlasticityFccUpdate::initQpStatefulProperties()
{
...
void
CrystalPlasticityFccUpdate::setInitialConstitutiveVariableValues()
{
  ...
  _previous_substep_H_shock = _H_shock_old[_qp];
}

void
CrystalPlasticityFccUpdate::setSubstepConstitutiveVariableValues()
{
  ...
  _H_shock[_qp] = _previous_substep_H_shock;
}

bool
CrystalPlasticityFccUpdate::calculateSlipRate()
{
  for (const auto i : make_range(_number_slip_systems))
  {
    ...
    if (std::abs(_slip_increment[_qp][i]) * _substep_dt > 1000.0*_slip_incr_tol)
    {
...
void
CrystalPlasticityFccUpdate::calculateEquivalentSlipIncrement(
...
void
CrystalPlasticityFccUpdate::calculateConstitutiveSlipDerivative(
...
bool
CrystalPlasticityFccUpdate::areConstitutiveStateVariablesConverged()
{
...
void
CrystalPlasticityFccUpdate::updateSubstepConstitutiveVariableValues()
{
  // Would also set substepped dislocation densities here if included in this model
...
void
CrystalPlasticityFccUpdate::cacheStateVariablesBeforeUpdate()
{
  _slip_resistance_before_update = _slip_resistance[_qp];
  _disloc_h_before_update = _disloc_h[_qp];
  _H_before_update = _H[_qp];
  _H_shock_before_update = _H_shock[_qp];
}

void
CrystalPlasticityFccUpdate::calculateStateVariableEvolutionRateComponent()
{
  std::vector<RealVectorValue> pnormal;
  pnormal=CrystalPlasticityFccUpdate::calplanenorm(_crysrot[_qp]);
  ...
  _disloc_h_increment=std::abs(std::accumulate(absslip.begin(),absslip.end(),0.0))*(_k1*std::pow(_disloc_h_before_update,0.5)-_k20*_disloc_h_before_update);
  ...
  for (const auto i : make_range(_number_slip_systems))
  {
    ...
    _H_increment += -_eta*(Pnormal.doubleContraction(_H_shock_before_update))*Pnormal*std::abs(_slip_increment_old[_qp][i]);
    }
  }
  for (const auto i : make_range(_number_slip_systems))
  {
...
CrystalPlasticityFccUpdate::updateStateVariables()
{
  ...
  for (const auto i : make_range(_number_slip_systems))
  {
    ...
    Pnormal=outer_product(pnormal[i],pnormal[i]);
    // _H[_qp]+=_H_increment;
    _slip_resistance[_qp][i] = _gss_initial+2.48*std::pow(10.0,-7.0)*86.0*std::pow(10.0,3.0)*(std::pow(std::max(0.0,_disloc_density[_qp]*0.125),0.5)+std::pow(0.1*std::max(Pnormal.doubleContraction(_H[_qp]),0.0),0.5));
    _gd[_qp][i] = 2.48*std::pow(10.0,-7.0)*86.0*std::pow(10.0,3.0)*(std::pow(0.1*std::max(Pnormal.doubleContraction(_H[_qp]),0.0),0.5));
    // _slip_resistance[_qp][i] = _gss_initial;
